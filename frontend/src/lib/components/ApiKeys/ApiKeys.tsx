...
export default class ApiKeys extends LoadableComponent<{}, State> {
    ...
    protected renderReady() {
        ...
        const columns = [
            {
                width: "250px", className: "api-keys-column-id", dataIndex: 'uid', key: 'uid', render: (text, row: Token, index) => {
                    return <><span className="api-keys-key-id">{text}</span>
            ...
            {
                width: "140px", className: "api-keys-column-actions", title: "Actions", dataIndex: 'actions', render: (text, row: Token, index) => {
                    return <>
    ...
    generateButton() {
        return (<Button type="primary" icon={<PlusOutlined/>} loading={"NEW" === this.state.loading} onClick={async () => {
            let newToken = {
                uid: this.newToken("", 6),
