...
export function MappingEditor({entity, onChange, closeDialog}: IMappingEditorProps) {
    ...
    let tableColumns = [
        {
            ...
            render: (val, mapping: Mapping, index) => {
                if (mapping == null) {
                    return <Button icon={<PlusOutlined/>} onClick={() => {
                        setCurrentMappings([...currentMappings, new Mapping("", "", 'move')])
                    }}>Add New Field Mapping</Button>
                }
                return <JsonPointerInput initialValue={mapping.srcField} validator={jsonPointerValidator} onChange={(val) => mapping.srcField = val}/>
            }
        },
        {
            width: "200px",
            ...
            render: (val, mapping: Mapping, index) => {
                ...
                return <Align horizontal="center"><Select dropdownMatchSelectWidth={false} className="mapping-editor-select-transform" size="small" value={mapping.action} onChange={(val) => {
                    mapping.action = val;
                    ...
                    {Object.entries(MAPPING_NAMES).map(([key, val]) => {
                        return <Select.Option value={key} key={key}>{val}</Select.Option>
                    })}
                </Select></Align>
            }
        },
        {
            ...
            render: (val, mapping: Mapping, index) => {
                ...
                return <JsonPointerInput initialValue={mapping.dstField} validator={jsonPointerValidator} onChange={(val) => mapping.dstField = val}/>
            }
        },
        {
            width: "5%",
            ...
            <Button type="primary" loading={saving} onClick={async () => {
                ...
                try {
                    for (let mapping of  currentMappings) {
                        if (!isValidJsonPointer(mapping.srcField) || (mapping.action !== 'erase' && !isValidJsonPointer(mapping.dstField))) {
    ...
    >
        ...
        <Table size="small" pagination={false} className="mapping-editor-table" columns={tableColumns} dataSource={[...currentMappings, null]}/>
    </Modal>);
}


function JsonPointerInput(props: { initialValue: any, onChange: (val: string) => void, validator: (val: string) => string}) {
    ...
    return <>
        <Input type="text" className="mapping-editor-json-pointer" defaultValue={props.initialValue} onChange={onChange} size="small" contentEditable={true}/>
